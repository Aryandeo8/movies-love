...
export const Logincard = () => {
  ...
  const [showPassword, setShowPassword] = useState(false); // Initialize as boolean
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('http://localhost:4000/api/user/register', { email, password })
      .then(result => {
        console.log(result.data); // Fix typo here
        if (result.data.success) {
          console.log('Login successful');
          navigate('/display'); // Correctly use navigate
        } else {
        ...
        <form onSubmit={handleSubmit} className='space-y-6'>
          ...
          <div className="flex items-center justify-center">
            ...
            <p className="text-sm text-gray-300 mt-4 text-center">
              Already have an account? <a href="/login" className="text-white hover:text-black">Login</a>
            </p>
          </div>
